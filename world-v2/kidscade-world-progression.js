...
function ensureProgress(){
  ...
  const s=loadSave(),raw=s.progression||{};
  progressCache={
    energy:clamp(Number(raw.energy??100),0,Math.max(50,Number(raw.maxEnergy)||100)),
    maxEnergy:Math.max(50,Number(raw.maxEnergy)||100),
...
function inventory(world){const cs=carrying(world);if(cs)return cs.inventory;const s=loadSave();return s.inventory||(s.inventory={});}
...
function spendForWork(world,kind){
  const p=ensureProgress(),cost=WORK_COST[kind]||0,toolKey=TOOL_FOR[kind];
  if(p.energy<cost){toast('체력이 부족해요. 집의 침대에서 쉬어 보세요.');return false;}
  if(toolKey){const def=TOOL[toolKey],t=p.tools[toolKey];if(!t||t.dur<=0){toast(`${def.name}가 필요해요. 농장 제작대에서 만들어 보세요.`);return false;}t.dur=Math.max(0,t.dur-1);}
...
function wrapInteraction(e,kind){const i=e?.interaction;if(!i||typeof i.action!=='function'||i.action.__progressWrapped)return;const original=i.action;function wrapped(t,w){if(!spendForWork(w,kind))return false;return original(t,w);}wrapped.__progressWrapped=true;i.action=wrapped;}
function patchWorkInteractions(world){for(const e of world.entities.all()){if(!e.active||!e.interaction)continue;if(e.type==='tree'&&!e.data?.workDestroyed)wrapInteraction(e,'chop');else if(e.type==='rock'&&!e.data?.workDestroyed)wrapInteraction(e,'mine');else if(e.type==='bug'&&!e.data?.caught)wrapInteraction(e,'net');else if(e.type==='fishing-spot')wrapInteraction(e,'fish');}}
...
function patchCrops(world){const p=ensureProgress();PLOT_TYPES.forEach((type,i)=>{const e=world.entities.get(`work-crop-${i+1}`);if(!e)return;e.data.cropType=type;e.data.__progressManaged=true;e.render=cropRender;const st=cropState(p,e.id,type);if(st.phase==='growing'&&Date.now()>=st.readyAt){st.phase='ripe';persistProgress(p);}setCropInteraction(world,e,st);});}

function installStarter(world){const p=ensureProgress(),inv=inventory(world),hasTool=Object.values(p.tools).some(t=>t&&t.dur>0),basic=(inv.wood||0)+(inv.stone||0);if(hasTool||basic>=5||world.entities.get('starter-mat-1'))return;const C=K.CarryFishing;if(!C?.spawnPickup)return;[['wood',2410,990],['wood',2490,1025],['wood',2570,985],['stone',2650,1028],['stone',2730,990],['stone',2810,1025]].forEach((v,i)=>C.spawnPickup(world,v[0],v[1],v[2],2,{id:`starter-mat-${i+1}`}));}
...
function ensurePanel(world){
  ...
  panel.querySelector('.wv2close').onclick=()=>closeCraft(world);panel.addEventListener('pointerdown',e=>{if(e.target===panel)closeCraft(world);});panel.addEventListener('click',e=>{const b=e.target.closest('[data-tool]');if(b)craft(world,b.dataset.tool);});root.addEventListener('keydown',e=>{if(e.key==='Escape'&&panel.classList.contains('open')){e.preventDefault();e.stopImmediatePropagation();closeCraft(world);}},true);return panel;
}
...
function openCraft(world){renderPanel(world);ensurePanel(world).classList.add('open');setWorldInput(world,false);}
...
function install(world){if(!world||world.__progressionInstalled)return false;world.__progressionInstalled=true;runtime.set(world,{regen:0});ensureProgress();ensurePanel(world);extendItems();installStation(world);installStarter(world);patchBed(world);world.events.on('update',({dt})=>{extendItems();patchWorkInteractions(world);patchCrops(world);patchBed(world);regen(world,dt);updateStatus(world);});updateStatus(world);return true;}
...
K.Progression={installed:true,TOOL,CROP,WORK_COST,install,craft,openCraft,closeCraft,spendForWork,restoreEnergy,ensureProgress};
autoInstall();
})(window);
