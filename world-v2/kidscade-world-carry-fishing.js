...
function ensureState(world){let s=states.get(world);if(s)return s;const save=K.Storage?.load?.()||{};s={held:null,inventory:{wood:0,stone:0,crop:0,fish:0,bug:0,potato:0,carrot:0,tomato:0,...(save.inventory||{})},fishing:null};states.set(world,s);return s;}
...
function pickUp(world,e){const st=ensureState(world);if(st.held){toast(`이미 ${nameOf(st.held.item)}을(를) 들고 있어요.`);return false;}st.held={item:e.data.item,qty:e.data.qty||1,source:e};e.visible=false;e.active=false;actorOf(world)?.play?.('smile',320);tone(620,.08,'sine',.03);toast(`${nameOf(st.held.item)}을(를) 들었어요. Q로 내려놓을 수 있어요.`);return true;}
...
function dropHeld(world){const st=ensureState(world),h=st.held;if(!h)return false;const p=world.player,x=p.centerX+(actorOf(world)?.facing||1)*34,y=p.y+p.h-4;if(h.source){h.source.x=x-h.source.w/2;h.source.y=y-h.source.h/2;h.source.visible=!isInside(world);h.source.active=!isInside(world);}else spawnPickup(world,h.item,x,y,h.qty);st.held=null;tone(260,.07,'triangle',.025);toast(`${nameOf(h.item)}을(를) 내려놓았어요.`);return true;}
...
function scanYields(world){for(const e of world.entities.all()){if(e.type==='tree'||e.type==='rock'){if(e.data?.workDestroyed&&!e.data.__carryYieldSpawned){e.data.__carryYieldSpawned=true;spawnPickup(world,e.type==='tree'?'wood':'stone',e.centerX,e.y+e.h-5,1);}}else if(e.type==='crop'){if(e.data?.__progressManaged)continue;if(e.data?.harvested&&!e.data.__carryHarvestSeen){e.data.__carryHarvestSeen=true;spawnPickup(world,'crop',e.centerX,e.y+e.h-3,1);}else if(!e.data?.harvested)e.data.__carryHarvestSeen=false;}else if(e.type==='bug'){if(e.data?.caught&&!e.data.__carryCaughtSeen){e.data.__carryCaughtSeen=true;spawnPickup(world,'bug',e.centerX,e.centerY,1);}else if(!e.data?.caught)e.data.__carryCaughtSeen=false;}}}
function updateFishing(world){const st=ensureState(world),f=st.fishing;if(!f)return;const now=performance.now();if(f.phase==='cast'&&now-f.phaseStart>=650){f.phase='wait';f.phaseStart=now;f.biteAt=now+1200+Math.random()*1900;}else if(f.phase==='wait'&&now>=f.biteAt){f.phase='bite';f.phaseStart=now;f.biteUntil=now+950;tone(1050,.10,'square',.04);toast('입질! 지금 E / Space!');}else if(f.phase==='bite'&&now>=f.biteUntil){tone(160,.12,'square',.025);endFishing(world,'물고기가 도망갔어요.');}else if(f.phase==='reel'&&now-f.phaseStart>=720){endFishing(world);holdVirtual(world,'fish',1);actorOf(world)?.play?.('smile',520);toast('물고기를 잡았어요! 보관 상자로 가져가 보세요.');}}
