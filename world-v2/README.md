...
### Tiny Farm · Town 지형 패스
- Kenney Tiny Farm/Town의 16×16 packed atlas를 World v2의 48px 그리드에 정확히 3배 확대
- Tiny Farm: 잔디 / 밭 흙 / 물
- Tiny Town: 길
- 런타임에서 불투명도·평균색·질감을 분석해 지형 타일 후보를 자동 선별
- 투명 오브젝트/캐릭터 타일이 반복 바닥으로 선택되지 않도록 98.5% 이상 불투명 셀만 후보로 사용
- 타일 로딩 전 또는 실패 시 기존 BQ3 grass/path/soil/water 렌더로 자동 폴백
- 타일 준비 완료 시 배경 캔버스만 다시 생성하므로 충돌·농사·낚시 좌표는 유지
...
## 현재 완료 범위
현재 **핵심 생활 루프**는 연결되었습니다. World v2를 기본 생활 월드로 사용해도 되는 구조까지 왔습니다. 남은 항목은 기존 루프를 완성하기 위한 필수 기능이라기보다 콘텐츠 확장에 가깝습니다.
...
기존 v1 파일은 현재 기본 진입점에서 사용되지 않습니다. PR 병합 전 비교/회귀 확인용으로만 보존합니다.
