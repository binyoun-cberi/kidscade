...
test('World v3 loads local Three.js and the v2 storage/avatar bridge',()=>{
  ...
  assert.match(html,/kidscade-world-v3\.js\?v=4/);
...
test('World v3 keeps the player 2D while the environment is 3D',()=>{
  ...
  assert.match(html,/id="avatarRuntime"/);
  assert.match(html,/\.\.\/avatar-studio\.html/);
  assert.match(runtime,/renderPreviewFrame/);
...
test('World v3 is the default entry and v2 remains reachable',()=>{
  assert.match(integration,/world-v3\/kidscade-world\.html\?v=4/);
