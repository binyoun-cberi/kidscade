...
test('Maratang Selfbar keeps only compact order, scale and action UI', () => {
  ...
  assert.doesNotMatch(html, /도움말|예시 주문|대기 주문|재료 카드|3D 마라탕 조리대/);
});

test('Maratang Selfbar runs a complete order to cook to serve loop', () => {
  assert.match(js, /TOTAL_CUSTOMERS = 5/);
