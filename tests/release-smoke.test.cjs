...
test('Nyam Universe release uses external Three.js and bounded 3D asset integration', () => {
  ...
  assert.match(game.href, /games\/low_nyam_universe\/냠냠 우주 여행\.html\?v=2/);
  const html = read(game.href);
  assert.match(html, /nyam-universe-v2\.js\?v=2/);
  ...
  assert.match(runtime, /LOW_POWER\?120:170/);
  assert.match(runtime, /index%3===0/);
});
