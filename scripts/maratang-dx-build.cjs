...
const versionedHref = `${gamePath}?v=3`;
...
if (!fs.existsSync(path.join(root, 'dist', gamePath))) throw new Error(`Missing built Maratang DX file: ${gamePath}`);

game.href = versionedHref;
fs.writeFileSync(catalogPath, `${JSON.stringify(catalog, null, 2)}\n`, 'utf8');
console.log(`[maratang-dx] catalog href set to ${versionedHref}`);
