...
function resolveBuildId() {
  const envBuild = String(process.env.CF_PAGES_COMMIT_SHA || process.env.GITHUB_SHA || '').trim();
...
function writeBuildVersion(buildId) {
  const id = String(buildId || '').trim();
  if (!id) throw new Error('Cloudflare build id is empty.');

  const indexPath = path.join(OUT, 'index.html');
  let html = fs.readFileSync(indexPath, 'utf8');
  if (!html.includes('__KIDSCADE_BUILD__')) {
    throw new Error('index.html is missing the __KIDSCADE_BUILD__ marker.');
  }
  html = html.replaceAll('__KIDSCADE_BUILD__', id);
  fs.writeFileSync(indexPath, html, 'utf8');

  fs.writeFileSync(
    path.join(OUT, 'kidscade-version.json'),
    JSON.stringify({ build: id, generatedAt: new Date().toISOString() }) + '\n',
    'utf8'
  );
}
...
async function optimizeFavicon() {
  ...
  await sharp(FAVICON_SOURCE)
    .resize({ width: 64, height: 64, fit: 'cover', position: 'centre' })
...
async function main() {
  ...
  if (faviconStats) {
    console.log(`- favicon: ${formatKiB(faviconStats.sourceBytes)} -> ${formatKiB(faviconStats.optimizedBytes)} (${faviconStats.output})`);
  }
  console.log('- timing exact 10.00 ranking client injected: yes');
  console.log(`- static auto-update build id: ${buildId.slice(0, 12)}`);
  console.log('- lobby/version manifest caching: disabled');
  console.log('- immutable cover/favicon caching with source-hash URLs: enabled');
