...
9. `game-launcher.js`
   - 게임 카드 클릭부터 iframe 열기, 세션 시작/종료, 최근 플레이 기록, 보상 계산 흐름을 담당한다.
   - 30초 보상 기준과 플레이 시간별 씨앗/경험치 계산을 `calculateReward()` 한 곳에 둔다.
...
10. `kidscade-game-sdk.js`
   - iframe 안쪽 게임이 플랫폼과 통신하는 공통 API다.
   - 공통 게임 메뉴, 종료, 재시작, 음소거 상태, 선택적 일시정지, 점수/최고기록, start/game-over 생명주기 이벤트를 제공한다.
   - 종료 요청과 생명주기 이벤트는 동일 origin 부모에게만 전달하고, 메인 bootstrap이 현재 game iframe에서 온 메시지만 수신한다.
   - 첫 도입 게임은 재난도시, 괴담 야간경비, 워드 블라스터, 다리야 버텨줘, 씨앗 배구다.
