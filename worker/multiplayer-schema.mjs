...
export async function ensureMultiplayerSchema(env) {
  ...
  if (!pending) {
    pending = db.exec(MULTIPLAYER_SCHEMA_SQL).catch(error => {
...
export async function multiplayerDatabaseHealth(env) {
  ...
  await env.DB.prepare('SELECT 1 AS ok FROM multiplayer_rooms LIMIT 1').first();
  return { ok: true, database: 'ready' };
}

export { MULTIPLAYER_SCHEMA_SQL };
