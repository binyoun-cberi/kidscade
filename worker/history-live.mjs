import { QUESTION_BANK } from '../data/history-live-question-bank.mjs';
...
function readRoomPlan(room){
  ...
  if(Array.isArray(raw))return {questions:raw,checkpoints:[]};
  const questions=Array.isArray(raw?.questions)?raw.questions:[];
  const checkpoints=Array.isArray(raw?.checkpoints)?raw.checkpoints.map(Number).filter(Number.isInteger):[];
  return {questions,checkpoints:[...new Set(checkpoints)].sort((a,b)=>a-b)};
}
...
async function createRoom(request,env){
  ...
  const questionCount=clampInt(body.questionCount,5,40,15), seconds=clampInt(body.secondsPerQuestion,8,30,12), checkpoints=normalizeCheckpoints(questionCount,body);
  await cleanup(env);
  const id=crypto.randomUUID(),code=await uniqueCode(env),token=randomToken(),hash=await sha256(token),now=Date.now(),order=shuffleIndexes(questionCount),plan={questions:order,checkpoints};
  ...
  return json({ok:true,code,hostToken:token,maxPlayers:MAX_PLAYERS,questionCount,secondsPerQuestion:seconds,checkpoints},201);
}
