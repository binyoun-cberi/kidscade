...
- `data/games.json`의 `title`을 실행 HTML 파일의 이름으로 사용합니다. 이름을 정리한 게임은 `games/<game-id>/<카드 제목>.html` 아래에 둡니다. 게임 ID와 기록 저장 키는 유지합니다.
- 게임을 하위 폴더로 옮길 때 공용 자산과 스크립트의 상대 경로도 함께 수정합니다. 루트 HTML 제한(114개)은 유지하며 현재 112개입니다.
