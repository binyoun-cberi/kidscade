...
function inspectTemp(){if(state.locked)return;clickSound();if(state.tests.temp){showMsg('체온은 이미 측정했습니다.');return}state.tests.temp=true;buttons.temp.classList.add('done');spendTime(2);const v=current().temp.toFixed(1)+'℃';addChip('체온',v,current().temp>=38?'warn':'');showMsg('체온 '+v,850)}
...
function inspectBlood(){if(state.locked||buttons.blood.disabled)return;clickSound();if(state.tests.blood){showMsg('혈액 검사는 이미 완료했습니다.');return}if(state.bloodKits<=0){if(!emergencyKits()){errorSound();showMsg('혈액 키트가 모두 소진되었습니다.',1400);return}}state.tests.blood=true;state.bloodKits--;buttons.blood.classList.add('done');spendTime(4);setToolAvailability();showMsg('혈액 분석 중…',600);setTimeout(function(){addChip('혈액',current().blood?'양성 (+)':'음성 (−)',current().blood?'warn':'');showMsg(current().blood?'항원 양성':'항원 음성',900);tone(current().blood?210:700,.09,'square',.03)},600)}
function inspectResp(){if(state.locked||buttons.resp.disabled)return;clickSound();if(state.tests.resp){showMsg('호흡 검사는 이미 완료했습니다.');return}state.tests.resp=true;buttons.resp.classList.add('done');spendTime(3);const bad=current().resp;addChip('호흡',bad?'비정상 · 위험':'정상',bad?'danger':'');showMsg(bad?'호흡 지수 비정상':'호흡 지수 정상',950)}
...
function restart(){state={weekIndex:0,caseIndex:0,trust:82,infection:9,score:0,bloodKits:0,records:[],tests:{},locked:false,started:true,time:0,streak:0,bestStreak:0,totalCorrect:0,totalCases:0,emergencyUsed:false};$id('reportModal').classList.remove('show');prepareWeek();showBriefing()}
...
buttons.id.addEventListener('click',inspectId);buttons.temp.addEventListener('click',inspectTemp);buttons.uv.addEventListener('click',inspectUv);buttons.blood.addEventListener('click',inspectBlood);buttons.resp.addEventListener('click',inspectResp);buttons.bag.addEventListener('click',inspectBag);
