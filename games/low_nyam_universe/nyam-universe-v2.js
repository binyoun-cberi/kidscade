...
(()=>{'use strict';
const $=id=>document.getElementById(id),T=THREE,KEY='nyamUniverse_v2',LEGACY_KEY='nyamUniverse_v1';
...
const foodRingMaterial=new T.MeshBasicMaterial({color:0xafffc1,transparent:true,opacity:.6,side:T.DoubleSide});
...
function shouldUseRealFood(type,near){
 const key=TYPE_ASSET[type];
 if(!key||!assetTemplates.has(key)||realFoodCount>=MAX_REAL_FOODS)return false;
 return near||Math.random()<(LOW_POWER?.18:.28);
}
...
function spawnFood(index,near=false){let s=stages[level],tier=index%3,sz=[.32,.8,1.8][tier]*(.85+Math.random()*.4);const useReal=shouldUseRealFood(s.types[tier],near);let g=model(s.types[tier],colors[index%colors.length],useReal);if(useReal)realFoodCount++;g.scale.setScalar(sz);let ang=Math.random()*Math.PI*2,dist=near?4+Math.random()*9:8+Math.random()*65;g.position.set(player.position.x+Math.cos(ang)*dist,0,player.position.z+Math.sin(ang)*dist);world.add(g);let r=new T.Mesh(ring,foodRingMaterial);r.scale.setScalar(sz*.85+.2);r.position.y=.025;g.add(r);foods.push({g,r,size:sz,name:s.items[tier],real:useReal})}
function build(){grace=2;death=null;stageStartCount=count;player.visible=true;for(let b of bots)b.halo.material.dispose();world.clear();foods=[];bots=[];effects=[];realFoodCount=0;scene.fog=new T.Fog(stages[level].sky,65,155);floor.material.color.setHex(stages[level].ground);player.position.set(0,0,0);player.rotation.set(0,0,0);decorate();showChapter();for(let i=0;i<(LOW_POWER?105:145);i++)spawnFood(i,i<(LOW_POWER?12:18));for(let i=0;i<(LOW_POWER?5:7);i++){let size=1.1+i*.46,g=critter(colors[i%6]);g.scale.setScalar(size);g.position.set(Math.cos(i)* (14+i*5),0,Math.sin(i)*(14+i*5));world.add(g);const halo=new T.Mesh(ring,new T.MeshBasicMaterial({color:0xff6969,transparent:true,opacity:.8,side:T.DoubleSide}));halo.position.y=.025;halo.scale.setScalar(1.1);g.add(halo);bots.push({g,halo,size,phase:Math.random()*6,hit:0})}tell(stages[level].name+' · 작은 것부터 냠냠!');cam.position.set(0,13,19)}
function save(){try{localStorage.setItem(KEY,JSON.stringify({level,logSize,count}));if(localStorage.getItem(LEGACY_KEY))localStorage.removeItem(LEGACY_KEY)}catch(e){}}function saved(){try{let raw=localStorage.getItem(KEY)||localStorage.getItem(LEGACY_KEY),s=JSON.parse(raw);if(s&&Number.isInteger(s.level)&&s.level>=0&&s.level<8&&Number.isFinite(s.logSize)&&s.logSize>=Math.log10(stages[s.level].min)&&s.logSize<=Math.log10(stages[s.level].max)&&Number.isFinite(s.count))return s}catch(e){}return null}
...
for(let i=foods.length-1;i>=0;i--){let f=foods[i],d=Math.hypot(f.g.position.x-player.position.x,f.g.position.z-player.position.z);f.r.visible=f.size<r*.82;f.r.material.opacity=.45+Math.sin(elapsed*3)*.15;if(d>90){f.g.position.set(player.position.x+(Math.random()-.5)*110,0,player.position.z+(Math.random()-.5)*110)}if(d<r*.7+f.size*.25&&f.size<r*.82&&eatCooldown<=0){let old=level;foods.splice(i,1);world.remove(f.g);if(f.real)realFoodCount=Math.max(0,realFoodCount-1);eat(f.size,f.name,f.g.position);if(level!==old||!playing)break;spawnFood(Math.floor(Math.random()*3),true)}}
