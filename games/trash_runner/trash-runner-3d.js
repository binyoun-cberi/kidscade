...
function removeEntry(entry){const i=trashEntries.indexOf(entry);if(i>=0)trashEntries.splice(i,1);scene.remove(entry.group);entry.group.traverse(n=>{if(n.geometry&&!n.userData.sharedGeometry)n.geometry.dispose?.();if(n.material&&!n.userData.sharedMaterial){const a=Array.isArray(n.material)?n.material:[n.material];a.forEach(m=>{m.map?.dispose?.();m.dispose?.()})}})}
